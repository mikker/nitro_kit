...
`AppNavigation` renders `nav[data-nk="app-navigation"][aria-label]` with optional unique `header` and `footer` regions around one required `body`. The body is a `ul[data-slot="app-navigation-body"]` and every entry is an `li`: `section(label: nil)` renders `li > span[data-slot="app-navigation-section-label"] + ul`, `divider` renders `li[role="separator"]`, and `spacer` renders one aria-hidden `li`. `item(text, href:, icon: nil, badge: nil, badge_color: :neutral, current: false, ...)` renders `li > a[data-slot="app-navigation-item-link"]` and carries the item's own `html:`, `aria:`, `data:`, and class escape onto that link. The body requires at least one item, at most one item is current, and a current item uses native `aria-current="page"` plus `data-state="current"` on the link.
