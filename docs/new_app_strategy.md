...
The template adds Nitro Kit, invokes `phlex:install` to establish the greenfield
Phlex view and component bases, then invokes `nitro_kit:install`. New
applications use Phlex for their application layout, route views, and reusable
UI. The Nitro generator owns the project-local skills and `AGENTS.md`. The
template does not copy Nitro components, controllers, authentication, teams,
billing, or product models.
...
An optional `nitro_kit:application` generator can later create an
application-owned Nitro composition: shell choice, root screen, and
authentication hooks. Keep that separate from installing the UI gem so
established teams can adopt Nitro Kit without adopting a new application
architecture.
