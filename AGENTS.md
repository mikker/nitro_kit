...
Nitro Kit is a gem-owned, Phlex-only UI system for Rails. Read `docs/agent_native_spec.md`, `docs/component_contracts.md`, and `STYLE_GUIDE.md` before editing components. Read `docs/agent_guide.md` and the matching file under `docs/patterns/` before changing an application interaction convention. `tk` is the source of truth for live status; completed implementation history remains in Git.
