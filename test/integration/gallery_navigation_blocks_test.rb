...
class GalleryNavigationBlocksTest < ActionDispatch::IntegrationTest
  ...
  test "catalog exposes the three accepted navigation blocks and defers progress steps" do
    ...
    assert_equal(
      BLOCKS.map { |slug| "/gallery/blocks/#{slug}" },
      entries.map { |entry| Gallery::Catalog.path_for(entry, routes: Rails.application.routes.url_helpers) }
    )
    assert_empty Gallery::Catalog.entries(kind: :block).map(&:slug) & %w[progress-steps]
  end

  test "every block page is direct class-free Phlex in light and dark themes" do
    BLOCKS.each do |slug|
      %w[light dark].each do |theme|
        get gallery_block_path(slug, theme:)
  ...
  test "real settings states use the layout with toolbars only around form and collection actions" do
    %w[profile security notifications integrations appearance].each do |state|
      get gallery_flow_path(slug: "settings", state:)
