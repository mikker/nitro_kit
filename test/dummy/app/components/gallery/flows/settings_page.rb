    ...
    class SettingsPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Settings flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
        ...
        render Section.new(
          ...
          description: "Block-composed profile, security, notification, integration, and appearance settings under real pressure."
        ) do
          render_example(
            slug: "settings-#{state}",
      ...
      def state_navigation
        nav(aria: { label: "Settings states" }, data: { gallery: "flow-states" }) do
