    ...
    class UsersPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Users flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
      ...
      def users_page_path(state:, page:, query: nil, status: nil)
        gallery_flow_path(**{ slug: entry.slug, state:, page:, query:, status: }.compact)
      end
      ...
      def state_navigation
        nav(aria: { label: "Users states" }, data: { gallery: "flow-states" }) do
