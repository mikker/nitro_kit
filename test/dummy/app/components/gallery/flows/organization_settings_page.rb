  ...
  module Flows
    class OrganizationSettingsPage < ExpandedPage
      ...
      def render_form_status(section, form)
        case state
        when "validation"
          section.status(NitroKit::Alert.new(variant: :error, id: "gallery-organization-settings-validation")) do |alert|
      ...
      def header_actions(actions)
        actions.button(
          ...
          href: gallery_flow_path(slug: "organization-overview", state: "active")
        )
