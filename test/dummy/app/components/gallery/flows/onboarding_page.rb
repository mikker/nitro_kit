    ...
    class OnboardingPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Onboarding flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
      ...
      def render_progress
        ...
        ol(data: { gallery: "flow-progress" }, aria: { label: "Onboarding progress" }) do
      ...
      def render_review_form
        ...
        dl(data: { gallery: "flow-summary" }) do
          dt { "Workspace" }
      ...
      def state_navigation
        nav(aria: { label: "Onboarding states" }, data: { gallery: "flow-states" }) do
