    ...
    class DashboardPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Workspace flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
        ...
        render Section.new(
          ...
          description: "Block-composed workspace overview with chart pressure across empty, healthy, degraded, loading, dense, and mobile states."
        ) do
          render_example(
      ...
      def state_navigation
        nav(aria: { label: "Dashboard states" }, data: { gallery: "flow-states" }) do
