    ...
    class SignInPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Authentication flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
        ...
        render Section.new(
          ...
          description: "AuthShell constrains one model-backed Card while the flow retains credentials, recovery navigation, Turbo replacement, and submission pressure."
        ) do
          render_example(
            slug: "sign-in-#{state}",
        ...
        render NitroKit::Card.new(id: "gallery-sign-in-card") do |card|
          ...
          card.footer do
            if state == "success"
              ...
              p do
                plain "New to Nitro? "
                a(href: entry_path(Gallery::Catalog.fetch!(kind: :flow, slug: "account-creation"))) do
                  "Create an account"
                end
              end
            end
      ...
      def state_navigation
        nav(aria: { label: "Sign-in states" }, data: { gallery: "flow-states" }) do
