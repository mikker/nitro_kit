    ...
    class AccountCreationPage < Page
      ...
      def page_template
        header(data: { gallery: "flow-header" }) do
          p(data: { gallery: "eyebrow" }) { "Registration flow" }
          h1 { entry.title }
          p { entry.description }
          state_navigation
        end
      ...
      def verification_path
        entry_path(Gallery::Catalog.fetch!(kind: :flow, slug: "email-verification"))
      end
      ...
      def state_navigation
        nav(aria: { label: "Account creation states" }, data: { gallery: "flow-states" }) do
